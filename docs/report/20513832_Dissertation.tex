...
{\large
Submitted 14 April 2026, in partial fulfilment of\\
...
In this dissertation, AI tools were used solely for supportive purposes.

Firstly, AI was also used to generate preliminary summaries of large volumes of content, with the aim of improving efficiency. These summaries were subsequently verified and refined by the author.

In addition, AI tools were employed to support language clarity and formatting, particularly in aligning the writing with academic conventions.
...
\begin{figure}[H]
	...
	\begin{subfigure}{0.45\linewidth}
		\includegraphics[width=\linewidth]{reasonable.png}
		\caption{Relevance: "This response is reasonable"}
	\end{subfigure}
	\hfill
	\begin{subfigure}{0.45\linewidth}
		\includegraphics[width=\linewidth]{professional.png}
		\caption{Tone appropriateness: "This response is professional"}
	\end{subfigure}
	
	\vspace{0.5cm}
	
	\begin{subfigure}{0.45\linewidth}
		\includegraphics[width=\linewidth]{helpful.png}
		\caption{Practicality: "This response helps advance cooperation"}
	\end{subfigure}
	\hfill
	\begin{subfigure}{0.45\linewidth}
		\includegraphics[width=\linewidth]{correct.png}
		\caption{Risk identification: "This response correctly identifies potential risks"}
	\end{subfigure}

	\caption{Participant evaluation results across key criteria (translated from Chinese)}
	\label{fig:charts}
\end{figure}

As shown in Figure~\ref{fig:charts}, the system received consistently high ratings across the evaluated dimensions.

For relevance ("this response is reasonable"), most of participants rated the responses as 4 or 5, indicating that the generated replies were generally appropriate and aligned with the original enquiry.

Similarly, tone appropriateness ("this response is professional") received strong evaluations, with most responses concentrated in the higher rating range. This suggests that the system is effective in maintaining a formal and suitable tone for business communication.
...
The system also performed well in risk identification ("this response correctly identifies potential risks"), with the majority of participants agreeing that the risks that identified were correct and reasonable. This shows that the risk analysis module effectively captured important risk categories.

Qualitative feedback further supports these findings. To provide deeper insight into user perceptions, selected participant comments are presented below (original responses were collected in Chinese and translated into English):

\begin{itemize}
    \item \textit{"The responses are clear and professional, and can be used directly in real business communication."}
    
    \item \textit{"It is very helpful for handling routine enquiries, especially when the information is complete."}
    
    \item \textit{"In some high-risk situations, the system should be more decisive instead of just asking for more information."}
    
    \item \textit{"The replies are polite and well-structured, but sometimes lack specific next steps."}
\end{itemize}
...
This dissertation has presented the design, development, and evaluation of an AI-driven trade communication assistant intended to support SMEs in international trade scenarios. The project was motivated by practical challenges faced by SMEs in cross-border business, including multilingual communication barriers, limited access to standardised documentation, and uncertainty in risk and compliance assessment.

To address these challenges, the project developed a prototype system centred on communication generation and risk identification, with document generation implemented as supporting functionality. The system adopts a modular client-server architecture and integrates trade fact extraction, risk analysis, decision routing, and response generation within a unified workflow.

The development process followed an iterative approach. Initial implementation established a functional system pipeline, while subsequent refinements focused on prompt design, response quality, risk-aware behaviour, and usability. Through pilot testing and user feedback, the system was progressively improved to produce more natural, professional, and context-aware responses.

The user study indicates that the system is capable of generating effective responses across a range of trade scenarios. In particular, it performs well in routine and medium-risk cases, although further improvement is required in handling complex and critical-risk situations.

\section{Main Contributions}

The main contributions of this project can be summarised as follows.

First, the project presents an integrated prototype for trade communication support, combining message understanding, communication generation, and risk identification within a single workflow. This addresses a limitation in existing approaches, which often focus on isolated functionalities rather than coordinated support.

Second, the project introduces a risk-aware communication mechanism, where generated responses are informed by both extracted trade facts and identified risk signals. This enables more context-sensitive and practically relevant responses in real-world trade scenarios.

Third, the project highlights the importance of iterative refinement in LLM-based systems. Through prompt engineering, post-processing, and user feedback, the system achieved improvements in clarity, tone consistency, localisation, and usability.

Finally, the project establishes a modular foundation for future AI-assisted trade support systems, demonstrating how multiple functionalities can be integrated into a coherent and extensible architecture.

\section{Final Remarks}

Overall, this project demonstrates that AI can support not only isolated business tasks, but also integrated trade communication workflows. As discussed in the literature, SMEs face multiple challenges in cross-border trade, including communication barriers, limited resources, and regulatory complexity \citep{itc2021,unctad2021,oecd2021}. 

This project addresses these challenges in a targeted manner. Communication barriers are mitigated through LLM-based multilingual response generation, enabling clearer and more professional interaction. Information asymmetry and incomplete inputs are partially addressed through structured trade fact extraction. In addition, regulatory uncertainty is considered through a risk identification mechanism that highlights potential compliance and transaction risks. 

Furthermore, in contrast to prior work that primarily focuses on isolated functionalities such as document automation or general language generation \citep{oecd_doc,ozturk2024}, this project demonstrates the feasibility of integrating communication generation, risk analysis, and basic document support within a unified system. This integrated approach reduces the need for users to rely on multiple tools and provides a more coherent workflow for handling trade communication.

At the same time, the project highlights several limitations. While the system improves efficiency and supports decision-making, it does not fully resolve regulatory complexity, and risk identification remains approximate due to the limitations of text-based analysis. In addition, large language models may produce inconsistent outputs, and document generation is not yet fully standardised. These limitations reflect broader challenges identified in the literature and indicate areas for further development.

Therefore, the system should be understood as an assistive tool rather than a replacement for human judgement. With further refinement, improved data integration, and more extensive real-world validation, such systems have the potential to become practical platforms that support SMEs in increasingly digitalised and complex global trade environments.
